...
const SingleHeader: FC<SingleHeaderProps> = ({
  ...
  if (categories?.nodes?.length == 1 && categories?.nodes?.[0]?.name == "Uncategorized") {
    nocategory = true
  }
          ...
          <>
            <nav aria-label="Breadcrumb">
              <ol className="breadcrumb hidden md:flex flex-wrap gap-1.5 items-center" itemScope itemType="https://schema.org/BreadcrumbList">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width="9" height="9" className="mt-1"><path d="M96 480c-8.188 0-16.38-3.125-22.62-9.375c-12.5-12.5-12.5-32.75 0-45.25L242.8 256L73.38 86.63c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0l192 192c12.5 12.5 12.5 32.75 0 45.25l-192 192C112.4 476.9 104.2 480 96 480z" fill="#5D6266" /></svg>

                {cate?.slice(0, -1)?.map((item: any, index: any) => (
                  <React.Fragment key={item.id || index}>
                    <li className="breadcrumb__item flex items-center" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                      <Link href={item?.uri ?? '/'} itemProp="item">
                        <span itemProp="name" className="text-[13px] text-[#5B5E61]">{item.name}</span>
                      </Link>
                      <meta itemProp="position" content="2" />
                    </li>
                    {cate && index < cate.length - 1 && <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className="mt-1" width="9" height="9"><path d="M96 480c-8.188 0-16.38-3.125-22.62-9.375c-12.5-12.5-12.5-32.75 0-45.25L242.8 256L73.38 86.63c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0l192 192c12.5 12.5 12.5 32.75 0 45.25l-192 192C112.4 476.9 104.2 480 96 480z" fill="#5D6266" /></svg>}
                  </React.Fragment>
                ))}

                <li className="breadcrumb__item" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem" aria-current="page">
                  <Link href={cate?.[cate.length - 1].uri}>
                    <span itemProp="name" className='text-[13px] text-[#5B5E61]'>{cate?.[cate.length - 1].name}</span>
