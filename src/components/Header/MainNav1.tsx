...
const MainNav1: FC<MainNav1Props> = ({ menuItems, title, description }) => {
	...
	const fetchCategories = async (value: string) => {
		try {
			const response = await axios.get('https://content.bestreviewsradar.com/wp-json/custom/v1/search-categories', {
				params: { keyword: value }
			})

			if (response.data.status === 'success') {
				setMatchedCategories(response.data.categories)
			} else {
	...
	const handleSubmit = (e: any) => {
		...
		if (searchValue.length > 0) {
			router.push('/search/posts/' + searchValue)
		}
	}
			...
			<div className="header-top bg-[#1b1d31] text-white py-3">
				<div className="max-w-[1200px] m-auto flex items-center justify-between">
					<div className="flex items-center">
						...
						>
							...
							{matchedCategories.length > 0 && (
								<ul className="absolute z-20 max-h-[200px] overflow-y-scroll bg-white border border-gray-300 w-full mt-2 rounded shadow-lg">
									{matchedCategories.map((category: any, index: any) => (
					...
					<div className="flex justify-end items-center gap-2">
						<Link href={"/"}>
							<img src="/images/us.png" className="w-[24px] md:w-[28px]" alt="" />
						</Link>
